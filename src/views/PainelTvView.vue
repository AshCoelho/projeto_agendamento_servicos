...
// ✅ enderecoId vindo da URL /tv/:enderecoId
...
// ✅ chave da última chamada pra saber se mudou (melhor que comparar só senha)
...
const buscarChamadas = async () => {
  ...
  try {
    // ✅ anti-cache: querystring + headers
    ...
    // ✅ chave “real” pra detectar mudança: horaChamada ou agendamentoId + hora
    ...
    // ✅ SEMPRE atualiza histórico (mesmo se senha não mudou)
    ...
    // ✅ só dispara voz/animação se mudou a chamada (key mudou)
    const mudou = key !== lastKey.value
    lastKey.value = key

    // ✅ atualiza a “senha atual” sempre (pra refletir guichê/cidadão também)
    senhaAtual.value = {
      numero: String(senha),
...
function start() {
  ...
  buscarChamadas()
  intervalChamada = setInterval(buscarChamadas, 1500) // mais “real time”
}
...
// ✅ se mudar o enderecoId na URL, reinicia tudo
