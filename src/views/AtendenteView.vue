  ...
  <div class="flex min-h-screen bg-[#f8f9fd] font-sans text-[#2d3748]">
    <aside class="w-64 bg-white border-r border-gray-100 flex flex-col items-center py-6">
      <div class="flex items-center gap-2 mb-10 self-start px-6">
        ...
        <div class="leading-3">
          <p class="text-[11px] font-black text-[#1e3a8a] tracking-tighter uppercase">Sistema de</p>
      ...
      <div class="mt-auto w-full px-6 pb-4">
        <div class="flex items-center gap-3 bg-gray-50 p-3 rounded-2xl mb-4 border border-gray-100">
          <div class="w-10 h-10 bg-[#2563eb] rounded-full flex items-center justify-center text-white font-bold shadow-sm">
        ...
        <button @click="handleLogout" class="flex items-center gap-2 text-red-500 font-bold text-xs uppercase tracking-tight">
    ...
    <main class="flex-1">
      <header class="flex justify-between items-center px-8 py-4">
        ...
        <div class="relative w-1/3">
          ...
          <input
            ...
            class="w-full bg-[#f1f4f9] border-none rounded-full py-2.5 pl-12 text-xs placeholder-gray-400 focus:ring-2 focus:ring-blue-100"
          />
        </div>
        ...
        <div class="flex items-center gap-8">
          <div class="text-right">
            <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest"
              >Guichê:</span
            >
            <span class="text-[#2563eb] font-black text-xl ml-1">
              {{ usuario?.guiche || '--' }}
            </span>
          </div>
          <div class="text-[#1e3a8a] font-black text-lg tracking-tighter">
            {{ relogio }}
          </div>
        </div>
      </header>

      <div class="px-8 grid grid-cols-3 gap-6 mb-8">
        <div
          class="bg-white p-6 rounded-[32px] shadow-sm flex justify-between items-start border-b-4 border-transparent hover:border-blue-500 transition-all"
        >
          <div>
          ...
          <div
            class="w-10 h-10 bg-gray-50 rounded-full flex items-center justify-center text-gray-300"
          >
        ...
        >
          <div>
            ...
            <div class="flex gap-2 mt-2">
              ...
              <span class="text-[9px] font-bold text-orange-500 uppercase">
          ...
          <div
            class="w-10 h-10 bg-gray-50 rounded-full flex items-center justify-center text-gray-300 shadow-inner"
          >
      ...
      <div class="px-8">
        <div class="bg-white rounded-[40px] shadow-sm border border-gray-50 p-4">
          ...
          <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-50">
              <tr v-for="(item, index) in agendamentosFiltrados" :key="index" class="group">
                ...
                <td class="px-6 text-right pr-6 flex gap-2 justify-end">
                  <v-btn
                    v-if="item.situacao === 'AGENDADO'"
                    color="warning"
                    class="text-white text-[11px] font-black uppercase px-6 py-2.5 rounded-xl transition"
...
export default {
  ...
  methods: {
    ...
    async handleFinalizar(id) {
      ...
      try {
        // Ajuste a rota de finalizar conforme seu backend (ex: PUT /finalizar/{id})
  ...
  computed: {
    ...
    if (this.abaAtiva === 'AGUARDANDO') {
      ...
      listaBase = this.agendamentosPorSec.filter(
        (a) => ['CHAMADO', 'EM_ATENDIMENTO', 'FALTOU'].includes(a.situacao)
      );
    } else {
